...
def print_state(active_set):
    ...
    max_w = get_w(max(active_set, key=get_w))
    min_w = get_w(min(active_set, key=get_w))
...
def get_neighbours(diff=1):
    r = range(-diff, diff+1)
    return [(x,y,z, w) for x in r for y in r for z in r for w in r if not x == y == z == w == 0]
